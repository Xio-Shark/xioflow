...
describe("shared-hooks capability table", () => {
  ...
  it("inject-subagent-context.py ships on all platforms with native sub-agent context delivery", () => {
    // Claude uses PreToolUse prompt mutation, Cursor its sub-agent hook, and
    // Codex SubagentStart.additionalContext — all three are push-capable.
    for (const [platform, hooks] of Object.entries(
      SHARED_HOOKS_BY_PLATFORM,
    )) {
      expect(
        hooks.includes("inject-subagent-context.py"),
        `${platform} should ship inject-subagent-context.py`,
      ).toBe(true);
    }
  });
