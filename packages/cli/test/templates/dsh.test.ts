...
import { collectCodexTemplates } from "../../src/configurators/codex.js";
...
describe("dsh collectDshTemplates", () => {
  ...
  it("renders .agents/skills/ files byte-identically to Codex's shared writes", () => {
    const dshFiles = collectDshTemplates();
    const codexFiles = collectCodexTemplates();
    for (const [key, content] of dshFiles) {
      if (!key.startsWith(".agents/skills/")) continue;
      expect(
        codexFiles.get(key),
        `${key} must be byte-identical to Codex's shared-skill write`,
      ).toBe(content);
    }
  });
