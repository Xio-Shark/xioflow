...
describe("uninstall() integration", () => {
  ...
  it("#8a empty managed sub-dirs and root dir are pruned (devin: no structured config)", async () => {
    // Devin has no hooks.json/settings.json/config.toml/package.json — every
    // manifest file is opaque and gets deleted, so the entire .devin/
    // tree should disappear, demonstrating both nested-subdir cleanup and
    // empty-platform-root cleanup.
    await init({ yes: true, devin: true, force: true });

    // Detect devin's actual config dir from manifest entries.
    const hashesBefore = loadHashes(tmpDir);
    const devinEntry = Object.keys(hashesBefore).find(
      (p) => !p.startsWith(".trellis/") && p !== "AGENTS.md",
    );
    if (!devinEntry) throw new Error("test fixture: no devin entries found");
    const devinRoot = devinEntry.split("/")[0];
    expect(fs.existsSync(path.join(tmpDir, devinRoot))).toBe(true);

    await uninstall({ yes: true });

    // Empty platform root dir should be removed.
    expect(fs.existsSync(path.join(tmpDir, devinRoot))).toBe(false);
  });
