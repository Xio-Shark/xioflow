...
describe("regression: Windows encoding (beta.10, beta.11, beta.16)", () => {
  ...
  it("[issue #190] Codex session-start hook forces UTF-8 stdout on Windows", () => {
    const content = getCodexHooks().find(
      (hook) => hook.name === "session-start.py",
    )?.content;
    expect(content, "codex session-start template should exist").toBeTruthy();
...
// =============================================================================
// regression: hook env gating + legacy Codex upgrade detection
// =============================================================================

describe("regression: hook env gating and legacy Codex upgrade detection", () => {
  const __dirname2 = path.dirname(fileURLToPath(import.meta.url));
  const repoRoot = path.resolve(__dirname2, "../../..");
  ...
  it("[trellis-hooks-env] all hook templates honor TRELLIS_HOOKS=0 / TRELLIS_DISABLE_HOOKS=1", () => {
    ...
    // Platform-specific Python session-start variants
    for (const [label, hooks] of [["codex", getCodexHooks()]] as const) {
