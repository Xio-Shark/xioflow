...
describe("isManagedPath", () => {
  ...
  it("rejects unrelated directories", () => {
    ...
    // Former supported platforms are no longer managed.
    expect(isManagedPath(".pi/extensions/trellis/index.ts")).toBe(false);
    expect(isManagedPath(".github/hooks/trellis.json")).toBe(false);
    expect(isManagedPath(".kiro/skills/start/SKILL.md")).toBe(false);
    expect(isManagedPath(".agent/workflows/start.md")).toBe(false);
  });
