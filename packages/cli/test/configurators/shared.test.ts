...
describe("resolveSkillsNeutral / resolveAllAsSkillsNeutral", () => {
  it("resolveSkillsNeutral produces byte-identical output for Codex and dsh", () => {
    const codexSkills = resolveSkillsNeutral(AI_TOOLS.codex.templateContext);
    const dshSkills = resolveSkillsNeutral(AI_TOOLS.dsh.templateContext);
    expect(codexSkills.length).toBe(dshSkills.length);
    for (let i = 0; i < codexSkills.length; i++) {
      expect(codexSkills[i].name).toBe(dshSkills[i].name);
      expect(codexSkills[i].content).toBe(dshSkills[i].content);
    }
  });
