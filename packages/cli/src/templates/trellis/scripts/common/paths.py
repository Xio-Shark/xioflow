...
ENV_DEVELOPER = "XIOFLOW_DEVELOPER"
ENV_DEVELOPER_LEGACY = "TRELLIS_DEVELOPER"
...
# =============================================================================
# Repository Root & Workflow Dir
# =============================================================================

def get_repo_root(start_path: Path | None = None) -> Path:
    """Find the nearest directory containing .xioflow/ (or legacy .trellis/) folder.
    ...
    Returns:
        Path to repository root, or current directory if neither found.
    """
    ...
    # Fallback to current directory if no workflow dir found
    return Path.cwd().resolve()


def get_workflow_dir_name(repo_root: Path | None = None) -> str:
    """Returns DIR_WORKFLOW (.xioflow) or DIR_WORKFLOW_LEGACY (.trellis) depending on existence."""
...
def get_workflow_dir(repo_root: Path | None = None) -> Path:
    """Returns absolute path to the project's active workflow directory."""
    if repo_root is None:
        repo_root = get_repo_root()
    return repo_root / get_workflow_dir_name(repo_root)
...
def get_developer(repo_root: Path | None = None) -> str | None:
    ...
    """
    env_name = os.environ.get(ENV_DEVELOPER, "").strip() or os.environ.get(ENV_DEVELOPER_LEGACY, "").strip()
    if env_name:
        return env_name
    ...
    local = _read_developer_file(get_workflow_dir(repo_root) / FILE_DEVELOPER)
    if local:
        return local
    ...
    return _read_developer_file(get_workflow_dir(main_root) / FILE_DEVELOPER)
...
def get_tasks_dir(repo_root: Path | None = None) -> Path:
    ...
    if repo_root is None:
        repo_root = get_repo_root()
    return get_workflow_dir(repo_root) / DIR_TASKS
...
def get_workspace_dir(repo_root: Path | None = None) -> Path | None:
    ...
    if developer:
        return get_workflow_dir(repo_root) / DIR_WORKSPACE / developer
    return None
