...
- **If the marker is absent**: hook injection didn't fire (Windows + Claude Code, `--continue` resume, fork distribution, hooks disabled, etc.). Find the active task path from your dispatch prompt's first line `Active task: <path>` (or run `python3 ./.xioflow/scripts/task.py current --source` as a fallback), then Read `<task-path>/implement.jsonl`, each listed file, `<task-path>/prd.md`, `<task-path>/design.md` if present, and `<task-path>/implement.md` if present before doing the work.

## Context

Before implementing, read:
- `.xioflow/workflow.md` - Project workflow
...
1. **Understand specs** - Read relevant spec files in `.xioflow/spec/`
...
- Spec layers: `.xioflow/spec/<package>/<layer>/`
- Shared guides: `.xioflow/spec/guides/`
- Guides: `.xioflow/spec/guides/`
