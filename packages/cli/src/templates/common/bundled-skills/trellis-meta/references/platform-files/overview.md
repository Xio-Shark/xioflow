...
Trellis connects the same local architecture to different AI tools. `.xioflow/` stores the shared runtime; platform directories store adapter files that define how each AI tool enters Trellis.

When a local AI modifies Trellis, it should distinguish two file categories first:

- **Shared files**: `.xioflow/workflow.md`, `.xioflow/tasks/`, `.xioflow/spec/`, `.xioflow/scripts/`.
...
- session-start injection of a `.xioflow/` overview.
...
To change behavior, inspect platform workflows/skills/commands and `.xioflow/workflow.md`.
...
1. Read `.xioflow/workflow.md` to confirm the shared flow.
...
5. If the change affects the shared flow, synchronize `.xioflow/workflow.md` or `.xioflow/spec/`.

Do not modify only platform files and forget the shared workflow. Do not modify only `.xioflow/workflow.md` and forget that platform entry points may still contain old descriptions.
