...
When the user wants to change Trellis phases, next-action hints, whether to create tasks, whether to use sub-agents, or when to check/wrap up, edit `.xioflow/workflow.md` first.
...
1. Find the relevant section in `.xioflow/workflow.md`.
...
4. Workflow-state changes only need an edit to the `[workflow-state:STATUS]` block in `.xioflow/workflow.md`. The hook is parser-only — it reads whatever you put in the block. Keep the opening and closing tags' STATUS strings identical (`[workflow-state:foo]…[/workflow-state:foo]`); mismatched STATUS pairs are silently dropped.
5. Make the AI reread `.xioflow/workflow.md`; do not keep using rules from the old conversation.
...
When you add a custom status (e.g. `in-review`), add a `[workflow-state:in-review]` block in `.xioflow/workflow.md` for the per-turn breadcrumb AND extend this route table — usually by editing the `/trellis:continue` command file (`.{platform}/commands/trellis/continue.md` or equivalent) to add a row that decides where to resume from. Without the route entry, `/trellis:continue` will fall through to a default branch and the user will not land on the step you intended.

## Notes

`.xioflow/workflow.md` is the local project workflow, not an immutable template. The user can adapt it to team habits. After editing it, platform entry files may still contain old descriptions, so inspect them too.
