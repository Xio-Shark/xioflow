...
2. `.xioflow/scripts/get_context.py`
3. `.xioflow/scripts/common/session_context.py`
4. `.xioflow/scripts/common/task_context.py`
5. `.xioflow/scripts/common/active_task.py`
...
| Source | Purpose |
| --- | --- |
| `.xioflow/workflow.md` | Workflow and next-action hints. |
| `.xioflow/tasks/<task>/prd.md` | Current task requirements. |
| `.xioflow/tasks/<task>/design.md` | Complex task technical design. |
| `.xioflow/tasks/<task>/implement.md` | Complex task execution plan. |
| `.xioflow/tasks/<task>/implement.jsonl` | Spec/research to read before implementation. |
| `.xioflow/tasks/<task>/check.jsonl` | Spec/research to read during checking. |
| `.xioflow/spec/` | Project specs. |
| `.xioflow/workspace/` | Session records. |
...
| Change hints on each user input | `[workflow-state:STATUS]` block in `.xioflow/workflow.md`. The `inject-workflow-state` hook is parser-only and reads the block verbatim. |
...
```jsonl
{"file": ".xioflow/spec/backend/index.md", "reason": "Backend conventions"}
{"file": ".xioflow/tasks/04-28-x/research/api.md", "reason": "API research"}
```
...
- `.xioflow/scripts/common/session_context.py`
...
python3 ./.xioflow/scripts/task.py list-context <task>
