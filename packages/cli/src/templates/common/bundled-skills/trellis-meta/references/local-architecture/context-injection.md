...
Trellis context injection aims to make AI read the right files at the right time instead of relying on model memory. In a user project, injection is implemented by `.xioflow/` scripts together with platform hooks, agents, and skills.

## Injected Context Types

| Type | Source | Purpose |
| --- | --- | --- |
| session context | `.xioflow/scripts/get_context.py` | Current developer, git status, active task, active tasks, journal, packages. |
| workflow context | `.xioflow/workflow.md` | Current Trellis flow and next action. |
| spec context | `.xioflow/spec/` + task JSONL | Specs that must be followed during implementation/checking. |
| task context | `.xioflow/tasks/<task>/prd.md`, `design.md`, `implement.md`, `research/` | Current task requirements, design, execution plan, and research. |
...
workflow-state is a lightweight hint injected around each user turn. Based on current task status, it selects a block from `.xioflow/workflow.md`, such as `no_task`, `planning`, `in_progress`, or `completed`.

If the user wants to change "what the AI should do next in a given state," edit the corresponding state block in `.xioflow/workflow.md` first.
...
```jsonl
{"file": ".xioflow/spec/backend/index.md", "reason": "Backend rules"}
```
...
Active task state lives in `.xioflow/.runtime/sessions/` and is isolated per session. Hooks try to resolve the context key from platform events, environment variables, transcript paths, or `TRELLIS_CONTEXT_ID`.
...
| Change per-turn workflow-state rules | `[workflow-state:STATUS]` block in `.xioflow/workflow.md`. The platform workflow-state hook parses these blocks verbatim and embeds no fallback text. |
| Change how sub-agents read context | Platform agent definitions, the `inject-subagent-context` hook, or agent preludes. |
| Change JSONL validation/display | `.xioflow/scripts/common/task_context.py`. |
| Change active task resolution | `.xioflow/scripts/common/active_task.py`. |
