...
3. `.xioflow/scripts/common/active_task.py`
4. `.xioflow/scripts/common/session_context.py`
5. `.xioflow/workflow.md`
...
3. Read the hook script and identify inputs, outputs, and called `.xioflow/scripts/`.
4. Modify hook behavior.
5. If the hook depends on workflow content, synchronize `.xioflow/workflow.md`.
...
If the hook ultimately calls `.xioflow/scripts/get_context.py` or `session_context.py`, editing the local script is usually more robust than hard-coding content in the hook.
...
- Hooks should read project-local `.xioflow/`; they should not depend on Trellis upstream source paths.
