...
2. **Agent definitions** inside the project at `.xioflow/agents/`: platform-agnostic role cards consumed by `trellis channel spawn --agent <name>`.
3. **Project configuration** in `.xioflow/config.yaml`: worker guard thresholds and other channel knobs.

## Core Paths

| Path | Purpose |
| --- | --- |
| `~/.xioflow/channels/<project>/<channel>/events.jsonl` | Per-channel append-only event log. Sequence-locked, replay-safe. |
| `~/.xioflow/channels/<project>/<channel>/<channel>.lock` | Channel-level write lock. |
| `~/.xioflow/channels/<project>/<channel>/<worker>.spawnlock` | Per-worker spawn lock used by the OOM guard. |
| `~/.xioflow/channels/<project>/<channel>/.seq` | Sequence sidecar for ordered event assignment. |
| `~/.xioflow/channels/_global/<channel>/...` | Channels created with `--scope global`. The project bucket is replaced by a shared key. |
| `.xioflow/agents/check.md` | Default Check Agent role definition consumed by `--agent check`. |
| `.xioflow/agents/implement.md` | Default Implement Agent role definition consumed by `--agent implement`. |
| `.xioflow/config.yaml` (`channel.*` block) | Worker guard thresholds and channel defaults. |
...
| Need | Edit location |
| --- | --- |
| Change default channel worker idle timeout | `channel.worker_guard.idle_timeout` in `.xioflow/config.yaml`. Accepts `5m`, `30s`, etc. Set `0` to disable idle cleanup. |
| Change live worker budget | `channel.worker_guard.max_live_workers` in `.xioflow/config.yaml`. Set `0` to disable the spawn-time budget check. |
...
| Change what the default Check or Implement worker does | Edit `.xioflow/agents/check.md` or `.xioflow/agents/implement.md`. These are platform-agnostic role cards; the channel runtime injects them when `--agent check|implement` is passed. |
| Add a new role card | Drop `<name>.md` into `.xioflow/agents/`. `trellis channel spawn --agent <name>` will pick it up. |
...
Precedence for the worker guard is: CLI flag > environment variable > `.xioflow/config.yaml` > built-in default. Built-in defaults are `idle_timeout: 5m` and `max_live_workers: 6`.

## Relationship To Other Local Layers

- **Workflow layer**: workflows that use channel dispatch (such as `channel-driven-subagent-dispatch`) instruct the main agent to call `trellis channel spawn --agent check` or `--agent implement` instead of a platform sub-agent. If `.xioflow/agents/check.md` or `implement.md` is missing, `trellis workflow --template <id>` prints a non-blocking warning at install time. Restore them with `trellis update` if they are deleted by accident.
...
- **Spec layer**: workers read `.xioflow/spec/` the same way the main session does. Channel runtime does not bypass spec context loading.
