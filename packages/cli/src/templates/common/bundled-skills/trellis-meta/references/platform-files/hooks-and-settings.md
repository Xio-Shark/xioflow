...
- workflow-state hook: parses `[workflow-state:STATUS]` blocks from `.xioflow/workflow.md` and emits the body matching the current task `status` on each user input. Parser-only; the script does not embed fallback content.
...
| `inject-workflow-state.py` | Parses `[workflow-state:STATUS]` blocks in `.xioflow/workflow.md` and emits the body matching the current task status. Falls back to `Refer to workflow.md for current step.` when no matching block exists. |
...
| Per-turn hint policy should change | `[workflow-state:STATUS]` block in `.xioflow/workflow.md`. The hook parses workflow.md verbatim — no script edit required. |
...
3. **Hooks read local `.xioflow/`, not upstream source**. `.xioflow/scripts/` and `.xioflow/workflow.md` in the user project are the default targets.
...
3. Manually run the `.xioflow/scripts/get_context.py` or `task.py current --source` command that the hook depends on.
4. Check whether active task state exists in `.xioflow/.runtime/sessions/`.
