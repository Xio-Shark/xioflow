...
The Trellis task system is stored entirely under `.xioflow/tasks/` in the user project. Each task is a directory containing requirements, context, research, state, and relationship information.
...
```bash
python3 ./.xioflow/scripts/task.py create "<child title>" --description "<one-line summary>" --slug <child-slug> --parent <parent-dir>
```

Link or unlink existing tasks with:

```bash
python3 ./.xioflow/scripts/task.py add-subtask <parent-dir> <child-dir>
python3 ./.xioflow/scripts/task.py remove-subtask <parent-dir> <child-dir>
```
...
```text
.xioflow/.runtime/sessions/<context-key>.json
```
...
```jsonl
{"file": ".xioflow/spec/cli/backend/index.md", "reason": "Backend conventions"}
{"file": ".xioflow/tasks/04-28-example/research/api.md", "reason": "API research"}
```
...
```bash
python3 ./.xioflow/scripts/task.py create "<title>" --description "<one-line summary>" --slug <slug>
python3 ./.xioflow/scripts/task.py start <task>
python3 ./.xioflow/scripts/task.py current --source
python3 ./.xioflow/scripts/task.py add-context <task> implement <file> <reason>
python3 ./.xioflow/scripts/task.py validate <task>
python3 ./.xioflow/scripts/task.py finish
python3 ./.xioflow/scripts/task.py archive <task>
```
...
| Need | Edit location |
| --- | --- |
| Change the default task template | `.xioflow/scripts/common/task_store.py` and task creation instructions. |
| Change status semantics | `.xioflow/workflow.md`, workflow-state hook logic, and task usage conventions. |
| Add task lifecycle actions | `hooks.after_*` in `.xioflow/config.yaml`. |
| Change context rules | Planning artifact guidance in `.xioflow/workflow.md` and related platform agent/hook instructions. |
| Change archive policy | `.xioflow/scripts/common/task_store.py` / `task_utils.py`. |
