...
Full guide in `.xioflow/workflow.md` (read on demand).

## Step 3: Guideline indexes
Discover packages + spec layers, then read each relevant index file.

```bash
{{PYTHON_CMD}} ./.xioflow/scripts/get_context.py --mode packages
cat .xioflow/spec/guides/index.md
cat .xioflow/spec/<package>/<layer>/index.md   # for each relevant layer
```
...
- **Active task status `in_progress`** → Phase 2 step 2.1. Load the step detail:
  ```bash
  {{PYTHON_CMD}} ./.xioflow/scripts/get_context.py --mode phase --step 2.1 --platform {{CLI_FLAG}}
  ```
...
Full rules + anti-rationalization table in `.xioflow/workflow.md`.
