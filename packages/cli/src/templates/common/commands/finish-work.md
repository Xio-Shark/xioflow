...
```bash
{{PYTHON_CMD}} ./.xioflow/scripts/get_context.py --mode record
```
...
Filter out paths under `.xioflow/workspace/` and `.xioflow/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
```bash
{{PYTHON_CMD}} ./.xioflow/scripts/task.py archive <task-name>
```
...
```bash
{{PYTHON_CMD}} ./.xioflow/scripts/add_session.py \
  --title "Session Title" \
  --commit "hash1,hash2" \
