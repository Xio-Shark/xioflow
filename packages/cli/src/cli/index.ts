...
function checkForUpdates(cwd: string): void {
  ...
  if (comparison > 0) {
    ...
    console.log(chalk.gray(`   Run: xioflow upgrade\n`));
  }
}
...
program
  .name("xioflow")
  .description(
    "AI Agent workflow and DAG task execution engine for Claude Code, Cursor, OpenCode, Codex, Devin, DSH",
  )
  .version(VERSION, "-v, --version", "output the version number");

program
  .command("init")
  .description("Initialize xioflow in the current project")
...
program
  .command("update")
  .description("Update xioflow configuration and commands to latest version")
...
program
  .command("upgrade")
  .description("Upgrade the global xioflow CLI package")
  .option(
    "--tag <tag>",
...
program
  .command("uninstall")
  .description(
    "Remove all xioflow files (managed platform files + .xioflow/) from this project",
  )
...
program
  .command("ablate")
  .description(
    "Temporarily remove all project xioflow surfaces with an external recovery transaction",
  )
  .option("-y, --yes", "Skip confirmation prompt")
...
program
  .command("mem")
  .description(
    "Search/recall AI conversation history across Claude Code, Codex, OpenCode, Pi (run 'xioflow mem help' for subcommands and flags)",
  )
