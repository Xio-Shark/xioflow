...
| No silent output loss | In-memory output is capped by `maxOutputBytes` (default 10 MiB); the full stream is spilled to `<domain>/artifacts/<opId>-stdout.log`, `fsync`ed, and hashed. `outputHash` is the SHA-256 of the full stream, `isTruncated` tells you the in-memory buffer was cut. |
